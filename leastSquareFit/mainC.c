...
int main(void) {
	...
	int noOfFuns = 3;
	leastSquareError *ls = leastSquareErrorAlloc(noOfFuns);
	(*ls).fitFun = &fitFun;
	leastSquareErrorSingularValueCalcConst(ls,x,y,dy);

	double stepSize = 0.05;
	double start = xTemp[0]-stepSize, end = xTemp[n-1]+stepSize;
	printf("# xFit \t yFit \t +Error \t -Error\n");
	for(double i = start; i < end; i += stepSize) {
		printf("%g \t %g \t %g \t %g\n",
			i,
			leastSquareErrorEval(ls,i),
			leastSquareErrorEvalPlus(ls,i),
			leastSquareErrorEvalMinus(ls,i));
	}

	leastSquareErrorFree(ls);
	gsl_vector_free(x);
	gsl_vector_free(y);
	gsl_vector_free(dy);
